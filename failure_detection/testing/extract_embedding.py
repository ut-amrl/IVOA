...
# of calculatig and saving the embedding (output of the one to the last fully
# connected layer with a dimension of 256) of all patches in the dataset
...
if __name__=="__main__":
    ...
    # Remove the last layer from the network to be left with the 256 dimensional
    # embedding at the output
    truncated_classifier = (
          nn.Sequential(*list(net.net.classifier.children())[:-1]))
    ...
    for cur_phase in phases:
        ...
        all_embeddings_np = all_embeddings.to(torch.device("cpu")).numpy()
        all_embeddings_var_np = (
