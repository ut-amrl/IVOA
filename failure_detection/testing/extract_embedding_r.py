...
# This script is a modified version of test_multi_class_model for the purpose 
# of calculatig and saving the embedding (out put of the feature extraction 
# layers with a dimension of 2048) of all patches in the dataset
...
# Helper function for saving the extracted embeddgins and the information of
# corresponding patches
def save_embeddings(target_dir, file_name, 
                    embeddings, patch_coords_x,
                    patch_coords_y, image_names):
   
    patch_coords_obj = {"x": patch_coords_x,
                        "y": patch_coords_y}
    
    results = {"image_name": image_names,
              "patch_coords": patch_coords_obj}
    
    results_json = json.dumps(results, indent=2)
    target_dir_append = target_dir + '/embeddings/'
    
    if not os.path.exists(target_dir_append):
        os.makedirs(target_dir_append)
    file = open(target_dir_append+file_name+'_patch_info.json', 'w');
    file.write(results_json)
    file.close()
    
    embedding_file_path = (target_dir_append+file_name+'_patch_embeddings.csv')
...
if __name__=="__main__":
    ...
    SAVE_EMBEDDINGS = True # Saves the output of the one to the laset fully
                           # connected layer for each input as well as the
                           # pixel coordinates of the patch and image name
    ...
    # Remove the last two fullly connected layers from the network to be left 
    # with the 2048 dimensional
    # embedding at the output
    truncated_classifier = (
          nn.Sequential(*list(net.net.classifier.children())[:-3]))
    net.net.classifier = truncated_classifier
    ...
    for cur_phase in phases:
        ...
        all_embeddings = torch.tensor([], dtype=torch.float, device=device)
        all_embeddings_var = torch.tensor([], dtype=torch.float, device=device)
        all_binary_labels = np.array([])
        all_multi_class_labels = np.array([])
        
        all_embeddings_np = np.array([], dtype=float)
        all_embeddings_var_np = np.array([], dtype=float)
        ...
        for i, data in enumerate(data_loaders[cur_phase], 0):
            ...
            particle_embeddings = torch.tensor([], dtype=torch.float,
                                                device = device)
            ...
            if CALC_UNCERTAINTY:
                for j in range(PARTICLE_NUM):
                    ...
                    with torch.set_grad_enabled(False):
                        ...
                        particle_embeddings = torch.cat((particle_embeddings,
                                                          outputs_softmax), 2)  
                ...
                # over all particles
                mean_embedding = torch.mean(particle_embeddings, 2)
                predicted_embedding_var = torch.var(particle_embeddings, 2)
                
                all_embeddings = torch.cat((all_embeddings, mean_embedding)
                                            , 0)
                all_embeddings_var = torch.cat((all_embeddings_var, 
                                                predicted_embedding_var)
                                            , 0)
            ...
            if i % checkpoint_num == checkpoint_num - 1:  # print every 
                ...
                # Empty torch tensors to cpu to free up GPU memory
                embeddings_np =( 
                                all_embeddings.to(torch.device("cpu")).numpy())
                embeddings_var_np = (
                            all_embeddings_var.to(torch.device("cpu")).numpy())
                all_embeddings = torch.tensor([], dtype=torch.float, 
                                              device=device)
                all_embeddings_var = torch.tensor([], dtype=torch.float, 
                                              device=device)
                
                
                if i == checkpoint_num - 1:
                    all_embeddings_np = embeddings_np
                    all_embeddings_var_np = embeddings_var_np
                else:
                    all_embeddings_np = np.append(all_embeddings_np,
                                                  embeddings_np,
                                                  axis=0)
                    all_embeddings_var_np = np.append(all_embeddings_var_np,
                                                  embeddings_var_np,
                                                  axis=0)
        ...
        if SAVE_EMBEDDINGS:
            patch_coords_x =  datasets['test'].patch_coord_left_x
            patch_coords_y =  datasets['test'].patch_coord_left_y
            image_names = datasets['test'].full_img_names
            
            save_embeddings(RESULTS_SAVE_DIR, RESULT_FILE_NAME, 
                        all_embeddings_np, patch_coords_x,
                        patch_coords_y, image_names)
