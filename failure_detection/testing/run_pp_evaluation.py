...
# This script is similar to run_evaluation.py with the difference that this
# script runs on the output of run_evaluation.py which is the processed
# images (class probabilites and uncertainty values as the result of 
# calculating the network output for overlapping windows)
# The purpose of this script is trying out different post processing methods on 
...
from data_loader.load_full_images_pp import FailureDetectionDataset
...
if __name__=="__main__":
    ...
    parser.add_argument('--save_dir', default=None, 
                        ...
                        required=True)
    parser.add_argument('--pre_results_dir', default=None, 
                        help='The path to the results before post processing', 
                        required=True)
    ...
    PRE_EVAL_RESULTS_DIR = args.pre_results_dir
    ...
    SAVE_VISUALIZATIONS = False  # heatmap visualization
    SAVE_PREDICTION_VISUALIZATIONS = True # annotation of predictions
    SAVE_PROCESSED_IMAGES = False # saves the class probabilities and 
                        #uncertainty (heatmaps) as separate files 
                        #to be used for post processecing
    PARTICLE_NUM = 2 # 50, 20
    ...
    for cur_phase in phases:
        ...
        for i, data in enumerate(data_loaders[cur_phase], 0):
            ...
            proc_imgs = data['proc_imgs']
            proc_img_uncertainty = data['proc_img_uncertainty']
            proc_img_meta_data = data['proc_img_meta_data']
            
           
            
            # For each bagfile check if the processed image meta data 
            # matches the expected values
            if (prev_bagfile_name != bagfile_name[0]):
                if EVAL_PIXEL_STRIDE != proc_img_meta_data["stride"]:
                    print('Expected stride size of', EVAL_PIXEL_STRIDE,
                          'but found it to be', proc_img_meta_data["stride"])
                    exit()
                if PATCH_SIZE != proc_img_meta_data["patch_size"]:
                    print('Expected patch size of', PATCH_SIZE,
                      'but found it to be', proc_img_meta_data["patch_size"])
                    exit()
            
         
           
            #evaluator.process_image(full_img[0, :, :, :])
            evaluator.load_processed_image(full_img[0, :, :, :],
                                           proc_imgs[0,:,:,:].numpy(), 
                                           proc_img_uncertainty[0,:,:].numpy())
            #predictions = evaluator.predict(patch_coords_left[0,:,:].numpy())
            predictions = evaluator.predict_filtered(
