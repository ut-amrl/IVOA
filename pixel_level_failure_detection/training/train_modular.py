...
def main(cfg, args, gpus):
  ...
  train_set_dict = {
    "train_tmp":[1007],
    "train_01_ganet_v0":[
          1006, 1008, 1009, 1011, 1013, 1014, 1016, 1018, 1019, 1021, 1023, 1024, 1025, 1026, 1028, 1029, 1030, 1031, 1033, 1034, 1035, 1036, 2006, 2008, 2009, 2011, 2013, 2014, 2016, 2018, 2019, 2021, 2023, 2024, 2025, 2026, 2028, 2029, 2030, 2031, 2033, 2034, 2035, 2036]
    }

  valid_set_dict = {
    "valid_tmp":[1007],
    "valid_01_ganet_v0":[1005, 1010, 1015, 1020, 2005, 2010, 2015, 2020]
  }
  ...
  writer = SummaryWriter(comment=cfg.MODEL.name)
  ...
  train_dataset = DepthErrorDataset(cfg.DATASET.root,
  ...
  for epoch in range(cfg.TRAIN.num_epoch):
    for cur_phase in phases:
      ...
      writer.add_scalar('Loss/{}'.format(cur_phase), epoch_loss, epoch)
